...
const useApi = (apiFunc) => {
  ...
  const request = useCallback(async (...args) => {
    if (loading) return { ok: false, message: "Request already in progress" };

    // Validate that apiFunc is actually a function
    if (typeof apiFunc !== 'function') {
      console.error('useApi: apiFunc must be a function, but received:', typeof apiFunc, apiFunc);
      setErrorStatus(true);
      setErrorMessage(`API function is not defined. Check your import and function name.`);
      setLoading(false);
    ...
    try {
      ...
      // Apisauce wraps the response in response.data, so response.data contains the actual API response
      if (response?.data?.respondStatus === "SUCCESS") {
      ...
      // Also check if respondStatus is at root level (some API configurations)
      ...
      if (response?.data && !response?.data?.errorMessages && !response?.data?.error) {
        ...
        if (response.data.userDetails || response.data.requestedData || response.data.data || response.data.JWT_Token || response.data.jwtToken) {
          setData(response.data);
          setErrorStatus(false);
          setErrorMessage('');
          return { ok: true, data: response.data };
      ...
      if (response?.data?.errorMessages) {
        ...
        switch (errorMsg.ErrorType) {
          ...
          case "User.Error":
            setErrorMessage(errorMsg.Errors || 'You do not have permission to perform this action.');
      ...
      const errorMsg = response?.data?.message || response?.data?.error || 'Unexpected response format from server.';
  ...
  if (typeof apiFunc !== 'function') {
    console.error('useApi: apiFunc must be a function, but received:', typeof apiFunc, apiFunc);
    console.error('This usually means the function is not exported correctly or the import path is wrong.');
  }
